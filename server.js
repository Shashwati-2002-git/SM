...
app.get("/api/professional/:id", async (req, res) => {
  try {
    ...
    const result = await db.request()
      .input("HP_ID", sql.Int, id)
      .query(`
        SELECT HP_ID, HP_Name, HP_Desc, HP_Sp_Field, HP_Profile_URL, WD_Timing, SD_Timing
        FROM HP_Table
        WHERE HP_ID = @HP_ID
      `);
...
app.get("/api/counsellors", async (req, res) => {
  try {
    const db = await connectDB();
    const result = await db.request().query(`
      SELECT HC_ID, HC_Name, HC_Desc, HC_Profile_URL, HC_WD_Timing, HC_SD_Timing 
      FROM HC_Table
    `);
